program test_maxpool3d_layer
  ...
  maxpool_layer = maxpool(pool_width=pool_size, pool_height=pool_size, pool_depth=pool_size, stride=stride)

  if (.not. maxpool_layer % name == 'maxpool3d') then
    ok = .false.
    write(stderr, '(a)') 'maxpool3d layer has its name set correctly.. failed'
  end if

  if (maxpool_layer % initialized) then
    ok = .false.
    write(stderr, '(a)') 'maxpool3d layer should not be marked as initialized yet.. failed'
  end if
  ...
  if (.not. maxpool_layer % initialized) then
    ok = .false.
    write(stderr, '(a)') 'maxpool3d layer should now be marked as initialized.. failed'
  end if

  if (.not. all(maxpool_layer % input_layer_shape == input_shape)) then
    ok = .false.
    write(stderr, '(a)') 'maxpool3d layer input layer shape should be correct.. failed'
  end if

  if (.not. all(maxpool_layer % layer_shape == output_shape)) then
    ok = .false.
    write(stderr, '(a)') 'maxpool3d layer output layer shape should be correct.. failed'
  end if
  ...
  do k = 1, width / 2
    do j = 1, width / 2
      do i = 1, width / 2
        ! Since input is i*j*k, maxpool3d output must be stride*i * stride*j * stride*k
        if (.not. all(output(:,i,j,k) == stride**3 * i * j * k)) then
          ok = .false.
          write(stderr, '(a)') 'maxpool3d layer forward pass correctly propagates the max value.. failed'
        end if
      end do
    end do
  end do
  ...
  select type(this_layer => maxpool_layer % p); type is(maxpool3d_layer)
    do k = 1, width
      do j = 1, width
        do i = 1, width
          if (mod(i,2) == 0 .and. mod(j,2) == 0 .and. mod(k,2) == 0) then
            if (.not. all(sample_input(:,i,j,k) == this_layer % gradient(:,i,j,k))) gradient_ok = .false.
          else
            if (.not. all(this_layer % gradient(:,i,j,k) == 0)) gradient_ok = .false.
          end if
        end do
      end do
    end do
  ...
  if (.not. gradient_ok) then
    ok = .false.
    write(stderr, '(a)') 'maxpool3d layer backward pass produces the correct dL/dx.. failed'
  end if

  if (ok) then
    print '(a)', 'test_maxpool3d_layer: All tests passed.'
  else
    write(stderr, '(a)') 'test_maxpool3d_layer: One or more tests failed.'
    stop 1
  end if
