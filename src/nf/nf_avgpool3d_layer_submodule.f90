...
contains
  ...
  module subroutine init(self, input_shape)
    ...
    ! input_shape is expected to be (channels, width, height, depth)
  ...
  pure module subroutine forward(self, input)
    ...
    do concurrent (i = 1:iextent:self % stride, j = 1:jextent:self % stride, &
                   k = 1:kextent:self % stride, n = 1:self % channels)
      ii = (i - 1) / self % stride + 1
      jj = (j - 1) / self % stride + 1
      kk = (k - 1) / self % stride + 1

      iend = min(i + self % pool_width - 1, input_width)
      jend = min(j + self % pool_height - 1, input_height)
      kend = min(k + self % pool_depth - 1, input_depth)

      ! Compute the average over the pooling region.
      self % output(n, ii, jj, kk) = sum(input(n, i:iend, j:jend, k:kend)) &
        / ((iend - i + 1) * (jend - j + 1) * (kend - k + 1))
    end do
  ...
  pure module subroutine backward(self, input, gradient)
    ...
    integer :: channels, pooled_width, pooled_height, pooled_depth
    ...
    pooled_depth   = size(gradient, dim=4)

    ! The gradient for average pooling is distributed evenly over the pooling window.
    do concurrent (n = 1:channels, i = 1:pooled_width, j = 1:pooled_height, k = 1:pooled_depth)
      ...
      kstart = (k - 1) * self % stride + 1
      kend   = min(kstart + self % pool_depth - 1, size(input, dim=4))
