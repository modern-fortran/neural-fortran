module nf_input4d_layer

  !! This module provides the `input4d_layer` type.

  use nf_base_layer, only: base_layer
  implicit none

  private
  public :: input4d_layer

  type, extends(base_layer) :: input4d_layer
    real, allocatable :: output(:,:,:,:)
  contains
    procedure :: init
    procedure :: set
  end type input4d_layer

  interface input4d_layer
    pure module function input4d_layer_cons(output_shape) result(res)
      !! Create a new instance of the 4-d input layer.
      !! Only used internally by the `layer % init` method.
      integer, intent(in) :: output_shape(4)
        !! Shape of the input layer
      type(input4d_layer) :: res
        !! 4-d input layer instance
    end function input4d_layer_cons
  end interface input4d_layer
