module nf_avgpool2d_layer
    ...
    type, extends(base_layer) :: avgpool2d_layer
        ...
        integer :: pool_size(2)  ! Pooling window size (height, width)
        integer :: stride(2)     ! Stride (height, width)
    ...
    interface avgpool2d_layer
        pure module function avgpool2d_layer_cons(pool_size, stride) result(res)
            ...
            integer, intent(in) :: pool_size(2)
                !! Height and width of the pooling window.
            integer, intent(in) :: stride(2)
                !! Stride of the pooling window (height, width).
